...
TMP_LOCATION ?= $(THIS_DIR)tmp

BIN_DIR=$(BIN_LOCATION)
TEST_EXE=$(patsubst %.cpp,$(BIN_LOCATION)/%.bin,$(notdir $(TEST_SOURCES)))
DEMO_EXE=$(patsubst %.c,$(DEMO_BIN_LOCATION)/%.bin,$(notdir $(DEMO_SOURCES)))
EXECUTABLES=$(TEST_EXE) $(DEMO_EXE)

DIRECTORIES=$(BUILD_LOCATION) $(OBJ_LOCATION) $(SRC_OBJ_LOCATION) $(PRIV_OBJ_LOCATION) $(PRIV2_OBJ_LOCATION) $(TEST_OBJ_LOCATION) $(DEMO_OBJ_LOCATION) $(BIN_LOCATION) $(DEMO_BIN_LOCATION) $(DATA_LOCATION) $(TMP_LOCATION)
...
MY_TMP_DATA_ARCHIVE=$(TMP_LOCATION)/$(MY_DATA_BASENAME).archive
MY_TMP_NORMALIZED_DATA_ARCHIVE=$(TMP_LOCATION)/$(MY_NORMALIZED_DATA_BASENAME).archive
...
$(BIN_LOCATION)/%.bin: $(TEST_OBJ_LOCATION)/%.o $(SRC_OBJECTS) $(PRIV_OBJECTS) $(PRIV2_OBJECTS)
	$(CPP) $^ -o $@ $(LDFLAGS)
